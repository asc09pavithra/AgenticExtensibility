...
export default function CreateCopyJobDialog(props) {
    ...
    async function loadDefaultOptionsAndProfile() {
        ...
        const booleanOptions = [
            ...
            if (copyProfile !== null) {
                ...
                copyProfile.base.definitions.forEach(item => {
                ...
                copyProfile.base.definitions.sort(function (a, b) {
                    var textA = a.optionName.toUpperCase();
                    var textB = b.optionName.toUpperCase();
                    return (textA < textB) ? -1 : (textA > textB) ? 1 : 0;
                });
                setProfile(copyProfile.base.definitions);
            }
        }
        catch (error) {
    ...
    function createTicket() {
        ...
        Object.entries(usedOptions.current).map(([key, value]) => {
            if (value !== null && value !== "" && usedOptions.current[key] != undefined) {
                if (key.includes('.')) { // Nested properties. Watermark, stamp options, etc.
                    const [parentKey, childKey] = key.split('.');
                    
                    if (!ticket.ticket.copyOptions[parentKey]) {
                        ticket.ticket.copyOptions[parentKey] = {};
                    }
                    
                    ticket.ticket.copyOptions[parentKey][childKey] = value;
                } else { // All other properties
                    ticket.ticket.copyOptions[key] = value;
                }
            }
        })
    ...
    async function handleValidateTicket() {
        ...
        try {
            console.log(ticket.ticket.copyOptions)
            response = await copyJobService.postCopyJobTicketHelper(ticket.ticket.copyOptions)
    ...
    async function handleStartJob() {
        ...
        try {
            let response = await copyJobService.createCopyJob(props.agentId, ticket)
            console.log("create CopyJob response- " + JSON.stringify(response));

            if (null != response) {
                props.setSelectedCopyJob(response.copyJobId);
                props.postAlert('success', 'Created Copy Job');
            }

            if (null !== props.onStartCopyClose) {
                props.onStartCopyClose();
            }
        }
        catch (error) {
            props.postAlert('error', 'Unable to create Copy Job  - ' + error.cause.message);
        }
    ...
    return (
        <React.Fragment>
            ...
            { Object.entries(defaultOptions).length === 0 || Object.entries(profile).length === 0 ? 
                <AppAlert open={props.open} handleClose={props.onClose} severity={'error'} message={'Unable to create Copy Ticket. Please verify a solution is installed with an active solution token.'} />
            :
            <Dialog fullWidth maxWidth="md" open={props.open} onClose={props.onClose}>
                <DialogTitle>{"Create New Copy Job"}</DialogTitle>
                <Grid style={{ marginTop: 10 }} container>
                    <Grid item xs={12}>
                        <Paper elevation={2}>
                            <AppBar position="static" color="inherit">
                                <Tabs value={value} onChange={handleChange} aria-label="copy job agents" {...baseTabStyles}>
                                    <Tab label="Basic Copy Options" {...a11yProps(0)} />
                                    <Tab label="Advanced Copy Options" {...a11yProps(1)} />
                                    <Tab label="Solution Context" {...a11yProps(2)} />
                                </Tabs>
                            </AppBar>
                ...
                <DialogActions>
                    <SDKButton
                        disabled={false}
                        buttonlabel="Cancel"
                        primaryToolTip="Exit Create Copy Job"
                        secondaryToolTip="Unable to Exit Create Copy Job"
                    ...
                    <SDKButton
                        ...
                        primaryToolTip="Start a new Copy Job"
